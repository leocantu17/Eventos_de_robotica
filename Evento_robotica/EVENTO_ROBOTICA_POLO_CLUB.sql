...
BEGIN
	SELECT E.CODIGO_EQUIPO,E.NOMBRE,A.NOMBRE AS NOMBRE_ASE,C.TIPO FROM EQUIPO E JOIN ASESOR A ON E.ID_ASESOR=A.ID_ASESOR JOIN CATEGORIA C ON E.ID_CATEGORIA=C.ID_CATEGORIA WHERE NOMBRE_INSTITUCION=(SELECT I.NOMBRE_INSTITUCION FROM INSTITUCION I JOIN CUENTAS C ON I.ID_CUENTA=C.ID_CUENTA WHERE I.ID_CUENTA=CODIGO_CUENTA);
END//
DELIMITER ;
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
DELIMITER //
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
DELIMITER //
CREATE PROCEDURE EQUIPO_EVENTO(ID_EVENTO INT,PUESTO VARCHAR(20))
BEGIN
	IF(PUESTO='SUPER') THEN
		...
		JOIN EQUIPO E ON E.NOMBRE_INSTITUCION=I.NOMBRE_INSTITUCION JOIN ASESOR A ON E.ID_ASESOR=A.ID_ASESOR JOIN CATEGORIA C ON E.ID_CATEGORIA=C.ID_CATEGORIA WHERE EV.CODIGO_EVENTO=ID_EVENTO;
    ELSE
		SELECT E.NOMBRE,A.NOMBRE AS NOMBRE_ASE,C.TIPO,E.NOMBRE_INSTITUCION,E.NIVEL,E.CODIGO_EQUIPO FROM EVENTO EV JOIN INVOLUCRADAS INV ON EV.CODIGO_EVENTO=INV.CODIGO_EVENTO JOIN INSTITUCION I ON INV.NOMBRE_INSTITUCION=I.NOMBRE_INSTITUCION
		JOIN EQUIPO E ON E.NOMBRE_INSTITUCION=I.NOMBRE_INSTITUCION JOIN ASESOR A ON E.ID_ASESOR=A.ID_ASESOR JOIN CATEGORIA C ON E.ID_CATEGORIA=C.ID_CATEGORIA JOIN JUECES J ON E.ID_CATEGORIA=J.ID_CATEGORIA  WHERE EV.CODIGO_EVENTO=ID_EVENTO;
    END IF;
END//
DELIMITER ;
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
DELIMITER //
CREATE PROCEDURE CALIFICAR_EQUIPO(CALIFICACION INT, EQUIPO_CALIF INT,OUT MENSAJE VARCHAR(30))
BEGIN
	DECLARE PROYECTOS INT;
    DECLARE PARTICIPANTES INT;
	SET PROYECTOS=(SELECT COUNT(CODIGO_PROYECTO) FROM PROYECTO WHERE CODIGO_EQUIPO=EQUIPO_CALIF);
    SET PARTICIPANTES=(SELECT COUNT(NUMERO_ESCOLAR) FROM PARTICIPANTES WHERE CODIGO_EQUIPO=EQUIPO_CALIF);
		IF(PARTICIPANTES>=1 AND PROYECTOS>=1) THEN
			UPDATE EQUIPO SET PUNTOS=(CAST(ROUND(CALIFICACION/3,2) AS DECIMAL(4,2))) WHERE CODIGO_EQUIPO=EQUIPO_CALIF;
            SET MENSAJE='OK';
		ELSE
			SET MENSAJE='NO';
        END IF;
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
DELIMITER //
CREATE PROCEDURE DETALLES_EQUIPO(CODIGO_CUENTA_P INT,PUESTO VARCHAR(30))
BEGIN
	IF(PUESTO='PARTICIPANTE') THEN
    ...
    ELSEIF(PUESTO='ASESOR') THEN
		SELECT E.CODIGO_EQUIPO,E.NOMBRE,E.PUNTOS,A.NOMBRE AS NOMBRE_ASESOR,C.TIPO,E.NOMBRE_INSTITUCION,E.NIVEL FROM EQUIPO E JOIN CATEGORIA C ON E.ID_CATEGORIA=C.ID_CATEGORIA JOIN ASESOR A ON E.ID_ASESOR=A.ID_ASESOR WHERE A.ID_CUENTA=CODIGO_CUENTA_P;
    ELSE
		SELECT E.CODIGO_EQUIPO,E.NOMBRE,E.PUNTOS,A.NOMBRE AS NOMBRE_ASESOR,C.TIPO,E.NOMBRE_INSTITUCION,E.NIVEL FROM EQUIPO E JOIN CATEGORIA C ON E.ID_CATEGORIA=C.ID_CATEGORIA JOIN ASESOR A ON E.ID_ASESOR=A.ID_ASESOR JOIN INSTITUCION I ON E.NOMBRE_INSTITUCION=I.NOMBRE_INSTITUCION WHERE I.ID_CUENTA=CODIGO_CUENTA_P;
    END IF;
END//
DELIMITER ;  

DELIMITER //
CREATE FUNCTION NOMBRE_EQU (ID_EQUIPO INT) RETURNS VARCHAR (100) DETERMINISTIC
BEGIN
	DECLARE NOMBRE_EQU VARCHAR(100);
	SELECT NOMBRE INTO @NOMBRE_EQU FROM EQUIPO WHERE CODIGO_EQUIPO = ID_EQUIPO;
RETURN @NOMBRE_EQU;
END// 
DELIMITER ;
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
DELIMITER //
CREATE TRIGGER EQUIPOS 
AFTER INSERT ON EQUIPO
FOR EACH ROW
BEGIN
	UPDATE EVENTO SET EQUIPOS=EQUIPOS+1;
END;
DELIMITER ;
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
DELIMITER //
CREATE PROCEDURE RECUPERAR_CONTRASENA(C_CORREO VARCHAR(100),OUT C_CONTRASENA VARCHAR(20),OUT MENSAJE VARCHAR(20))
BEGIN
   IF EXISTS(SELECT 1 FROM CUENTAS WHERE CORREO=C_CORREO) THEN
	SET C_CONTRASENA=(SELECT CONTRASENA FROM CUENTAS WHERE CORREO=C_CORREO);
	SET MENSAJE='OK';
   ELSE
	SET MENSAJE='ERR';
   END IF;
